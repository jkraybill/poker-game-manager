...
export function assertActionSequence(actions, expectedSequence, options = {}) {
  ...
  while (sequenceIndex < expectedSequence.length && actionIndex < actions.length) {
    ...
    if (matchesActionPattern(actualAction, expectedAction)) {
      ...
      expect.fail(`Expected action ${JSON.stringify(expectedAction)} at position ${actionIndex}, got ${JSON.stringify(actualAction)}`);
    }
  ...
  if (sequenceIndex < expectedSequence.length) {
    expect.fail(`Missing expected actions: ${JSON.stringify(expectedSequence.slice(sequenceIndex))}`);
  }
}
...
function matchesActionPattern(actualAction, expectedPattern) {
  if (expectedPattern.action && actualAction.action !== expectedPattern.action) {
    return false;
  }
  
  if (expectedPattern.amount !== undefined && actualAction.amount !== expectedPattern.amount) {
    return false;
  }
  
  if (expectedPattern.playerId && actualAction.playerId !== expectedPattern.playerId) {
    return false;
  }
...
export function assertActionsOccurred(actions, expectedActions, options = {}) {
  ...
  expectedActions.forEach(expectedAction => {
    const found = actions.some(action => matchesActionPattern(action, expectedAction));
...
export function assertBettingRounds(actions, expectedRounds, options = {}) {
  ...
  expectedRounds.forEach((expectedRound, index) => {
    ...
    if (expectedRound.minActions) {
      expect(roundActions[index].length).toBeGreaterThanOrEqual(expectedRound.minActions);
    }
    
    if (expectedRound.maxActions) {
      expect(roundActions[index].length).toBeLessThanOrEqual(expectedRound.maxActions);
    }
...
export function assertAllInScenario(actions, winners, expectedOutcome, options = {}) {
  ...
  const allInActions = actions.filter(action => action.action === Action.ALL_IN);
  expect(allInActions.length).toBeGreaterThanOrEqual(expectedOutcome.minAllIns || 1);
...
export function assertTournamentState(gameState, expectedState, options = {}) {
  ...
  if (expectedState.chipLeader) {
    ...
    const chipLeader = Object.values(gameState.players).find(p => p.chips === maxChips);
    expect(chipLeader.id).toBe(expectedState.chipLeader);
  }
  
  if (expectedState.totalChips) {
    const totalChips = Object.values(gameState.players).reduce((sum, p) => sum + p.chips, 0);
