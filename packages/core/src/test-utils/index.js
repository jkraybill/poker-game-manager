...
import { createTestTable, createChipStackTable, createWaitHelper, cleanupTables } from './tableFactory.js';
import { setupEventCapture, waitForGameStart, waitForHandEnd } from './eventCapture.js';
import { DeckBuilder } from './deckBuilder.js';
import { PLAYER_TYPES, StrategicPlayer, TournamentPlayer, STRATEGIES } from './players.js';
...
export function createTestScenario(config = {}) {
  ...
  // Create table
  const { manager, table } = chipAmounts.length > 0 
...
export function createSplitPotScenario(config = {}) {
  const {
    playerCount = 3,
    identicalHands = [['8h', '9h'], ['8d', '9d'], ['8s', '9s']],
...
export async function executeBatchTests(testConfigs, options = {}) {
  ...
  if (parallel) {
    ...
    for (const config of testConfigs) {
      try {
        const result = await executePokerTest(config.setup, config.validation, config.options);
        results.push(result);
      } catch (error) {
        if (continueOnError) {
          results.push({ error });
