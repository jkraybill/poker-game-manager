...
describe('Standings Display (Issue #34)', () => {
  ...
  it('should separate active and eliminated players in standings', async () => {
    ...
    const handComplete = new Promise((resolve) => {
      ...
      table.on('player:eliminated', ({ playerId }) => {
        eliminatedCount++;
        const playerName = [alice, bob, charlie].find(p => p.id === playerId)?.name || playerId;
        console.log(`Player ${playerName} (${playerId}) eliminated (${eliminatedCount} total)`);
    ...
    const finalStandings = getFormattedStandings(table.players, eliminatedPlayers);
    
    console.log('Final standings:', {
      active: finalStandings.standings.map(p => ({ name: p.name, chips: p.chips })),
      eliminated: finalStandings.eliminated.map(p => ({ name: p.name, status: p.status })),
    ...
    console.log('✅ Issue #34 fix verified: Eliminated players properly separated from active standings');
  });
  ...
  it('should handle all players eliminated scenario', async () => {
    ...
    const handComplete = new Promise((resolve) => {
      table.on('player:eliminated', ({ playerId }) => {
        const playerName = [player1, player2].find(p => p.id === playerId)?.name || playerId;
        eliminatedPlayers.push({
          id: playerId,
          name: playerName,
          chips: 0,
