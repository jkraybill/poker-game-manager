...
describe('5-Player Complex Side Pots (v2)', () => {
  ...
  it('should handle complex side pot with multiple all-ins at different amounts', async () => {
    ...
    console.log('All actions:', actions.map(a => {
      ...
      return { name: player?.name, chips: player?.chipAmount, action: a.action, amount: a.amount };
    }));
    ...
    const allInPlayers = allIns.map(a => players.find(p => p.id === a.playerId));
    const allInChips = allInPlayers.map(p => p.chipAmount).sort((a, b) => a - b);
