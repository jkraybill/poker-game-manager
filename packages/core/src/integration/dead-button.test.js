...
describe('Dead Button Rules', () => {
  ...
  it('should implement dead button when player between button and blinds is eliminated', async () => {
    ...
    const blindTracking = {
      ...
      hand2: { sb: null, bb: null, button: null, deadButton: false, deadSB: false },
    };
    ...
    table.on('hand:started', (data) => {
      ...
      if (handCount === 1) {
        ...
        const activePlayerIds = players.filter(p => p.chips > 0).map(p => p.id);
        const buttonPlayerId = data.players[data.dealerButton];
        blindTracking.hand2.deadButton = !activePlayerIds.includes(buttonPlayerId);
