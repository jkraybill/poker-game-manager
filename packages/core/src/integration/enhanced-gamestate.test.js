...
describe('Enhanced GameState Validation Numbers', () => {
  ...
  it('should correctly calculate minRaise after multiple raises', async () => {
    ...
    if (raiserSecondAction) {
      ...
      expect(raiserSecondAction.bettingHistory.raiseHistory.length).toBeGreaterThan(0);
      expect(raiserSecondAction.bettingHistory.lastRaiseSize).toBeGreaterThan(0);
    }
  ...
  it('should handle all-in scenarios with proper max bet calculations', async () => {
    ...
    const shortStackAction = gameStatesRecorded.find(gs => gs.playerId === 'short');
    ...
    const bigStackAction = gameStatesRecorded.find(gs => gs.playerId === 'big');
    if (bigStackAction) {
