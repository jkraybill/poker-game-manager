  ...
  it('should not show eliminated players in active player list', async () => {
    ...
    table.on('hand:ended', () => {
      ...
      if (handCount === 1) {
        ...
        if (playerB) {
          console.log(`Player B still in table.players with chips: ${playerB.player.chips}`);
        } else {
    ...
    console.log('Post-elimination active players:', postEliminationActivePlayers);
    
    // Check if eliminated player is still in table.players
    const eliminatedPlayer = table.players.get('B');
    if (eliminatedPlayer) {
      console.log(`BUG: Eliminated player B still in table with chips: ${eliminatedPlayer.player.chips}`);
    }
    
    // The key test: eliminated players should not be in active standings
    if (postEliminationActivePlayers) {
      const eliminatedInActiveList = postEliminationActivePlayers.some(p => p.id === 'B' && p.chips === 0);
