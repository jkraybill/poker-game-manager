...
class ReproductionPlayer extends Player {
  ...
  getAction(gameState) {
    ...
    console.log(
      `${this.name} request #${logEntry.requestCount}: ${JSON.stringify(logEntry)}`,
    );
...
describe('ALL_IN Infinite Loop Bug (v4.4.4)', () => {
  ...
  it('should reproduce infinite loop with ALL_IN vs BET actions', () => {
  ...
  it('should reproduce reopenBetting bug with ALL_IN players', async () => {
    ...
    table.on('player:action', (data) => {
      ...
      console.log(
        `Action ${actionCount}: ${data.playerId} - ${data.action} ${data.amount || ''}`,
      );
    ...
    console.log(
      'Action log:',
      actionLog.map((a) => `${a.playerId}:${a.action}`),
    );
    console.log(
      `Short stack (${shortStack.name}) requests:`,
      shortStack.actionsLog.length,
    );
    console.log(
      `Big stack (${bigStack.name}) requests:`,
      bigStack.actionsLog.length,
    );
    ...
    if (shortStack.actionsLog.length > 1) {
      console.error(
        '🐛 BUG REPRODUCED: Short stack asked to act multiple times after ALL_IN',
      );
