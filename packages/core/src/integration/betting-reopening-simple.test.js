...
describe('Issue #32 - Betting Reopening Rules', () => {
  ...
  it('should NOT allow re-raise when all-in is less than minimum raise', async () => {
    ...
    table.on('action:invalid', ({ playerId, action, reason }) => {
      console.log(`INVALID ACTION: Player ${playerId} tried ${action} - ${reason}`);
    });
    ...
    class TestPlayer extends Player {
      ...
      getAction(state) {
        actionCount++;
        console.log(`\n--- Action ${actionCount} ---`);
        console.log(`Player ${this.name} to act. Current bet: ${state.currentBet}, My bet: ${state.players[this.id].bet}, Chips: ${state.players[this.id].chips}`);
        console.log(`Valid actions: ${state.validActions?.join(', ')}`);
        console.log(`Can raise: ${state.validActions?.includes(Action.RAISE) ? 'YES' : 'NO'}`);
        
        const action = this.strategy(state, this.id, roundActions);
        console.log(`${this.name} decides: ${action.action} ${action.amount || ''}`);
        return action;
      }
    }
    ...
    const p1Strategy = (state, playerId, prevActions) => {
      ...
      if (prevActions.length === 2) { // After SB and BB posts
        return {
          playerId,
          action: Action.RAISE,
          amount: 300,
      ...
      if (toCall > 0) {
        console.log(`P1 facing bet of ${state.currentBet}, need to call ${toCall}`);
        
        // Check if we can raise using validActions
        if (state.validActions.includes(Action.RAISE) && myState.chips > toCall * 2) {
    ...
    if (table.gameEngine) {
      const originalHandleAllIn = table.gameEngine.handleAllIn.bind(table.gameEngine);
      table.gameEngine.handleAllIn = function(player) {
        ...
        console.log(`  Will reopen betting: ${raiseIncrement >= minRaiseIncrement ? 'YES' : 'NO'}`);
    ...
    const p3AllIn = actions.find(a => a.playerId === 'p3' && a.action === Action.ALL_IN);
    if (p3AllIn) {
      console.log(`\nP3 went all-in for ${p3AllIn.amount}`);
      const p3AllInIndex = actions.indexOf(p3AllIn);
      const p1ActionsAfterAllIn = actions.slice(p3AllInIndex + 1).filter(a => a.playerId === 'p1');
      
      console.log('P1 actions after P3 all-in:', p1ActionsAfterAllIn);
      
      // Check if any of those actions were raises
      const p1RaisesAfterAllIn = p1ActionsAfterAllIn.filter(a => a.action === Action.RAISE);
