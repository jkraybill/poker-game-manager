...
describe('Race Condition - Simple Reproduction', () => {
  ...
  it('should demonstrate the timing issue between hand:ended and elimination processing', async () => {
    ...
    table.on('player:eliminated', ({ playerId }) => {
      ...
      const remainingChips = Array.from(table.players.values()).reduce(
        (sum, pd) => sum + pd.player.chips,
        0,
      );
    ...
    console.log(
      `hand:ended chip count: ${handEndedChipCount}/${totalChipsExpected}`,
    );
    ...
    if (handEndedPlayerCount > 1 && eliminationFired) {
      console.log(
        '🚨 RACE CONDITION: hand:ended fired while eliminated players still in table!',
      );
    }
  ...
  it('should show current behavior: hand:ended fires immediately after GameEngine', async () => {
    ...
    table.on('hand:ended', ({ winners }) => {
      const timeSinceComplete =
        eventSequence.length > 0 ? Date.now() - eventSequence[0].timestamp : 0;
      ...
      console.log(
        `📡 Table.hand:ended fired (+${timeSinceComplete}ms after GameEngine)`,
      );
    });
    ...
    table.on('player:eliminated', ({ playerId }) => {
      ...
      console.log(
        `🗑️  Table.player:eliminated fired (+${timeSinceComplete}ms after GameEngine)`,
      );
    });
    ...
    const handCompleteEvent = eventSequence.find(
      (e) => e.event === 'GameEngine.hand:complete',
    );
    const handEndedEvent = eventSequence.find(
      (e) => e.event === 'Table.hand:ended',
    );
