...
describe('Elimination Display Fixed', () => {
  ...
  it('should properly remove eliminated players from active standings', async () => {
    ...
    // Create players
    const playerA = new TestPlayer({ id: 'A', name: 'Player A (raiser)', strategy: 'raiser' });
    const playerB = new TestPlayer({ id: 'B', name: 'Player B (caller)', strategy: 'caller' });
    const playerC = new TestPlayer({ id: 'C', name: 'Player C (folder)', strategy: 'folder' });
