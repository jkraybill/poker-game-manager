...
describe('Big Blind Option Events (v2)', () => {
  ...
  it('should only emit action:requested once when big blind has option to check', async () => {
    ...
    const simpleStrategy = ({ player, toCall }) => {
      console.log(
        `${player.name} getAction called - strategyType: ${player.strategyType}, toCall: ${toCall}`,
      );

      if (player.strategyType === 'fold' && toCall > 0) {
    ...
    buttonPlayer.strategyType = 'fold';
