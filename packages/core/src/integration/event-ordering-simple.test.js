  ...
  it('should fire player:eliminated after hand:ended', async () => {
          ...
          playerId === shortStack.id
            ...
            : playerId === medium.id
              ...
              : playerId === bigStack.id
                ...
                : 'Unknown';
        console.log(
          `EVENT: ${eventName} from ${playerName} - action: ${data?.action}, amount: ${data?.amount}`,
        );
      } else {
        console.log(
          `EVENT: ${eventName}`,
          data?.playerId || data?.winners?.length || '',
        );
      }
    });
    ...
    if (eliminationEvents.length > 0) {
      ...
      if (!shortStackData || shortStackData?.player.chips === 0) {
        console.log(
          'WARNING: Player has 0 chips but no elimination event fired',
        );
        // This might happen if they won or split the pot
        // Check if they were in the winners
        const wasWinner = events.winners.some(
          (w) => w.playerId === shortStack.id,
        );
        if (!wasWinner) {
          console.error(
            'BUG: Player lost with 0 chips but no elimination event!',
          );
