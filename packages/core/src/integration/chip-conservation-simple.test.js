...
describe('Chip Conservation Bug - Simple Reproduction', () => {
  ...
  it('should preserve total chips during hand:ended event', async () => {
    ...
    table.on('hand:ended', ({ winners }) => {
      ...
      if (difference !== 0) {
        console.log(`🚨 CHIP CONSERVATION VIOLATION: ${difference} chips ${difference > 0 ? 'MISSING' : 'EXTRA'}`);
      }
    });
  ...
  it('should demonstrate chip count changes over multiple events', async () => {
    ...
    // Add players
    const players = [
      new StrategicPlayer({ name: 'Player 1', strategy: STRATEGIES.alwaysCall }),
      new StrategicPlayer({ name: 'Player 2', strategy: STRATEGIES.alwaysCall }),
      new StrategicPlayer({ name: 'Player 3', strategy: STRATEGIES.alwaysCall }),
    ];
    ...
    ['hand:started', 'hand:ended', 'player:eliminated'].forEach(eventName => {
      table.on(eventName, (_data) => {
        const totalChips = Array.from(table.players.values())
          .reduce((sum, pd) => sum + pd.player.chips, 0);
        ...
        console.log(`${eventName}: ${totalChips}/${totalChipsExpected} chips (${table.players.size} players)`);
      });
    });
    ...
    chipCountLog.forEach((log, i) => {
      ...
      console.log(`${i + 1}. ${status} ${log.event}: ${log.totalChips} chips (diff: ${log.difference})`);
    });
    ...
    if (violations.length > 0) {
      ...
      violations.forEach(v => {
        console.log(`  ${v.event}: ${v.difference} chips ${v.difference > 0 ? 'missing' : 'extra'}`);
      });
    }

    // Final assertion: all events should preserve chip conservation
    const handEndedEvent = chipCountLog.find(log => log.event === 'hand:ended');
