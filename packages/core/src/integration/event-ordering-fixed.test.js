  ...
  it('should fire player:eliminated after hand:ended when player has 0 chips', async () => {
    ...
    class SimplePlayer extends Player {
      ...
      getAction(gameState) {
        ...
        console.log(`${this.name} decision: strategy=${this.strategy}, chips=${myState.chips}, toCall=${toCall}`);
        ...
        if (toCall > 0 && toCall > myState.chips) {
          console.log(`${this.name} cannot afford to call ${toCall}, going all-in with ${myState.chips}`);
          return {
            playerId: this.id,
