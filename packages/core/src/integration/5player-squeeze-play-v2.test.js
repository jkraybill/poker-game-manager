...
describe('5-Player Squeeze Play (v2)', () => {
  ...
  it('should handle SB squeeze play after raise and call', async () => {
    ...
    const squeezePlayStrategy = ({ player, gameState, toCall }) => {
      ...
      if (stackSize === 'utg' && gameState.currentBet === 20 && !player.hasActed) {
      ...
      if (stackSize === 'button' && toCall > 0 && toCall <= 60 && gameState.currentBet === 60 && !player.hasActed) {
        player.hasActed = true;
        return { action: Action.CALL, amount: toCall };
      }

      // SB (600 chips) squeezes after detecting raise and call
      if (stackSize === 'sb' && gameState.currentBet === 60 && !player.hasActed) {
    ...
    console.log('Actions:', actions.map(a => ({ action: a.action, amount: a.amount })));
